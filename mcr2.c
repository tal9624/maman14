...
{	
	...
	char **macConts = malloc(0 * sizeof(char*)); /*dynamicly initializing size of string array */
	char **macNames = malloc(0 * sizeof(char*)); /*dynamicly initializing size of string array */
	char buffer[LINELEN];
	char *kelet = argv[1];
	FILE *fpNew = NULL;
...
void saveName(char* buffer, char** macNames , char** macConts,int* maci,int i , int* mcrBool) 
{
	...
	*mcrBool = 1;
	(*maci)++;
	macNames = realloc(macNames,((*maci)+1)*sizeof(char*)); /* extending the array 																	for each macro */
	macConts = realloc(macConts,((*maci)+1)*sizeof(char*));
	macConts[*maci] =(char*)malloc(0*sizeof(char));
...
void saveCon(char* buffer , char** macConts,int maci,int i,int* mcrBool)
{
	...
	&& buffer[i+3] == 'm' && buffer[i+4] == 'c' &&  buffer[i+5] == 'r')) /* ...endmcr*/
		...
		macConts[maci] = realloc(macConts[maci],LINELEN*sizeof(char));
